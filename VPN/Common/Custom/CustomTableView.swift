...
public class CustomTableView: TPKeyboardAvoidingTableView, UITableViewDataSource, UITableViewDelegate{
//    private let gradientTableView = GradiantTableView()
//    gradientTableView.gradientLayer
        
//    let gradientTableView = GradiantTableView()
    func setGradientBackground(colors: [UIColor], startPoint: CGPoint, endPoint: CGPoint) {
           ...
           // Create a view to add the gradient layer as the background view
           let gradientBackgroundView = UIView(frame: bounds)
           gradientBackgroundView.layer.insertSublayer(gradientLayer, at: 0)
           
           // Set the gradient background view as the background view for the table view
